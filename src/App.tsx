...
import { formatCurrencyCop, formatDateTime, formatMinutes, formatRemainingClock, getRemainingMs } from './lib/format'
...
function App(): ReactElement {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [nextProfile, nextAtvs, nextOpenSessions, completedSessions] =
        await Promise.all([
  ...
  const handleAddAtv = useCallback(
    async (input: { name: string; plate?: string; baseMinutes: number; basePriceCop: number }) => {
  ...
  const handleUpdateRates = useCallback(
    async (atvId: string, input: { baseMinutes: number; basePriceCop: number }) => {
    ...
    <section className="panel">
      ...
      <div className="card-grid">
        {atvs.map((atv) => {
          ...
          const availabilityClass = !atv.active ? 'inactive' : isRunning ? 'busy' : isPaused ? 'paused' : 'ok'
...
function AtvAdminTab(props: {
  atvs: Atv[]
  canEdit: boolean
  onAddAtv: (input: { name: string; plate?: string; baseMinutes: number; basePriceCop: number }) => Promise<void>
  onUpdateRates: (atvId: string, input: { baseMinutes: number; basePriceCop: number }) => Promise<void>
  onToggleActive: (atvId: string, active: boolean) => Promise<void>
  onError: (message: string) => void
}): ReactElement {
  const { atvs, canEdit, onAddAtv, onUpdateRates, onToggleActive, onError } = props
  ...
  const [rateDrafts, setRateDrafts] = useState<Record<string, { baseMinutes: number; basePriceCop: number }>>({})
