-- Regla comercial de combos:
-- - Moto en combo: 10 min = 8000 COP (por bloques cerrados de 10)
-- - Brinca en combo: 15 min = 5000 COP (por bloques cerrados de 15)
-- Estas reglas aplican al cierre manual y automatico de sesiones.
    ...
    minutes_billed = public.calculate_billed_minutes(
      case
        when exists (
          select 1
          from public.combos c
          where c.brinca_session_id = public.brinca_sessions.id
            and c.status <> 'cancelled'
        ) then 15
        else base_minutes
      end,
